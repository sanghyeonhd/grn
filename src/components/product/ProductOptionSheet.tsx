      ...
      <Dialog open={showCartDialog} onOpenChange={() => setShowCartDialog(false)}>
        <DialogContent className="sm:max-w-md bg-white/90 backdrop-blur-sm">
          ...
          <DialogFooter className="sm:flex-row gap-2">
            <button
              className="flex-1 py-3 border border-black text-black"
              onClick={() => handleCartDialogClose(false)}
            >
              계속 쇼핑하기
            </button>
            <button
              className="flex-1 py-3 bg-[#2C2C2C] text-white"
              onClick={() => handleCartDialogClose(true)}
            >
              장바구니 보기
            </button>
          </DialogFooter>
