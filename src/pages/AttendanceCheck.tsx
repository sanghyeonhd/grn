...
const AttendanceCheck = () => {
  ...
  return <div className="min-h-screen bg-[#FDFBF4] px-4 py-6">
      ...
      <div className="space-y-8">
        <div className="bg-white p-6 rounded-lg shadow-sm space-y-8">
          ...
          <div className="grid grid-cols-7 gap-2">
            {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, i) => <div key={i} className="text-center py-2 text-sm text-gray-500">{day}</div>)}
            
            {Array(31).fill(null).map((_, i) => {
            const isChecked = checkedDates.includes(i + 1);
            const isToday = i + 1 === today;
            return <div key={i} className={`text-center py-2 text-sm relative ${isChecked ? 'text-red-500 font-medium' : ''} ${isToday ? 'font-bold' : ''}`}>
                  {i + 1}
                  {isChecked && <div className="absolute -bottom-1 right-1">
          ...
          <div className="bg-gray-50 p-6 rounded-lg">
            <div className="grid grid-cols-2 divide-x divide-gray-200">
              <div className="text-center px-4">
                <p className="text-gray-500 mx-0 px-0 text-xs">누적 참여 횟수</p>
                <p className="font-medium text-xl mt-1">{attendanceCount} 일</p>
              </div>
              <div className="text-center px-4">
                <p className="text-gray-500 text-xs px-0 mx-0">누적획득 포인트</p>
          ...
          <Button onClick={handleAttendanceCheck} className={`w-full ${checkedDates.includes(today) ? 'bg-gray-400 hover:bg-gray-400 cursor-not-allowed' : 'bg-[#2C2C2C] hover:bg-[#1a1a1a]'} text-white rounded-none h-12`} disabled={checkedDates.includes(today)}>
            {checkedDates.includes(today) ? '오늘은 이미 출석완료' : '출석 체크하기'}
          </Button>

          <Button onClick={handleLuckyDraw} className="w-full bg-white hover:bg-gray-50 text-black border border-gray-300 rounded-none h-12">
            행운 뽑기
          </Button>
        </div>
      </div>
      ...
      <Dialog open={showLuckyDrawDialog} onOpenChange={setShowLuckyDrawDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="text-center space-y-4 py-4">
            ...
            <Button onClick={() => setShowLuckyDrawDialog(false)} className="w-full bg-[#2C2C2C] hover:bg-[#1a1a1a] text-white rounded-none h-12">
              포인트 확인
            </Button>
          </div>
        </DialogContent>
